...
import { server, startHttpServer } from "./cron-channel.ts";
...
describe("cron-channel", () => {
  ...
  test("port is configurable via CRON_CHANNEL_PORT env var", () => {
    const { getPort } = require("./cron-channel.ts");
    const original = process.env.CRON_CHANNEL_PORT;
    try {
      process.env.CRON_CHANNEL_PORT = "9999";
      expect(getPort()).toBe(9999);
    } finally {
      if (original === undefined) delete process.env.CRON_CHANNEL_PORT;
      else process.env.CRON_CHANNEL_PORT = original;
    }
  });
  ...
  test("defaults to port 8790", () => {
    ...
    const original = process.env.CRON_CHANNEL_PORT;
    try {
      delete process.env.CRON_CHANNEL_PORT;
      expect(getPort()).toBe(8790);
    } finally {
      if (original !== undefined) process.env.CRON_CHANNEL_PORT = original;
    }
  });
